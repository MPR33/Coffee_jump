...
@export var coffee_rate := +0.06            # /s, la vitesse à laquelle ça descend en Coffee
...
func _on_transition_state_changed(state: bool) -> void:
	is_transitioning = state

func register_main_signals(main_node: Node) -> void:
	if main_node.has_signal("transition_state_changed"):
		main_node.transition_state_changed.connect(_on_transition_state_changed)

func _process(delta: float) -> void:
	# Met à jour la caféine selon le mode
	if game_started and not is_transitioning:
		if mode == Mode.DOODLE:
